import React, { useState } from "react";
import { Container, Card, Table, Form, Row, Col, Button, Badge } from "react-bootstrap";

export default function CotizacionesRealizadas() {
  const [busquedaCliente, setBusquedaCliente] = useState("");
  const [busquedaFecha, setBusquedaFecha] = useState("");

  const [cotizaciones, setCotizaciones] = useState([
    { id: 1, numero: "C001", cliente: "Juan Pérez", total: 95000, fecha: "2025-01-10", estado: "Aprobada" },
    { id: 2, numero: "C002", cliente: "María Rodríguez", total: 130000, fecha: "2025-01-15", estado: "Pendiente" },
    { id: 3, numero: "C003", cliente: "Carlos Ramírez", total: 78000, fecha: "2025-02-01", estado: "Rechazada" },
    { id: 4, numero: "C004", cliente: "Ana Torres", total: 114000, fecha: "2025-02-10", estado: "Aprobada" },
    { id: 5, numero: "C005", cliente: "Pedro Gómez", total: 65000, fecha: "2025-03-02", estado: "Pendiente" },
  ]);

  // 🔍 Filtrar por cliente o fecha
  const cotizacionesFiltradas = cotizaciones.filter((c) => {
    const coincideCliente = c.cliente.toLowerCase().includes(busquedaCliente.toLowerCase());
    const coincideFecha = busquedaFecha ? c.fecha === busquedaFecha : true;
    return coincideCliente && coincideFecha;
  });

  const getBadge = (estado) => {
    switch (estado) {
      case "Aprobada":
        return <Badge bg="success">Aprobada</Badge>;
      case "Pendiente":
        return <Badge bg="warning" text="dark">Pendiente</Badge>;
      case "Rechazada":
        return <Badge bg="danger">Rechazada</Badge>;
      default:
        return <Badge bg="secondary">{estado}</Badge>;
    }
  };

  // 🖨️ Reimprimir cotización
  const handleReimprimir = (cotizacion) => {
    const ventana = window.open("", "_blank");
    ventana.document.write(`
      <html>
        <head>
          <title>Cotización ${cotizacion.numero}</title>
          <style>
            body { font-family: Arial, sans-serif; margin: 40px; color: #222; }
            h1, h2 { color: #c00; text-align: center; }
            table { width: 100%; border-collapse: collapse; margin-top: 20px; }
            th, td { border: 1px solid #999; padding: 8px; text-align: center; }
            th { background-color: #f0f0f0; }
            .footer { text-align: center; margin-top: 30px; font-size: 13px; color: #555; }
            img { display: block; margin: 0 auto 10px; width: 80px; }
          </style>
        </head>
        <body>
          <img src="/imagenes/android-chrome-192x192.png" alt="Logo Gama Repuestos">
          <h1>GAMA REPUESTOS QUIBDÓ</h1>
          <h2>COTIZACIÓN ${cotizacion.numero}</h2>
          <p><strong>Cliente:</strong> ${cotizacion.cliente}</p>
          <p><strong>Fecha:</strong> ${cotizacion.fecha}</p>
          <table>
            <tr><th>Total</th><th>Estado</th></tr>
            <tr><td>$${cotizacion.total.toLocaleString()}</td><td>${cotizacion.estado}</td></tr>
          </table>
          <div class="footer">
            <p>Gracias por su preferencia 💪</p>
            <p>Calle 5 #12-34, Quibdó, Chocó | contacto@gamarepuestos.com</p>
          </div>
        </body>
      </html>
    `);
    ventana.document.close();
    ventana.print();
  };

  // 📄 Duplicar cotización
  const handleDuplicar = (cotizacion) => {
    const nuevaCotizacion = {
      ...cotizacion,
      id: cotizaciones.length + 1,
      numero: `C00${cotizaciones.length + 1}`,
      fecha: new Date().toISOString().split("T")[0],
      estado: "Pendiente",
    };
    setCotizaciones([...cotizaciones, nuevaCotizacion]);
    alert(`✅ Cotización ${cotizacion.numero} duplicada como ${nuevaCotizacion.numero}`);
  };

  return (
    <Container className="py-4">
      <Card className="p-4 bg-dark text-light shadow-lg border-0">
        <h2 className="text-center text-danger mb-4">Cotizaciones Realizadas</h2>

        {/* 🔍 Filtros */}
        <Form className="mb-4">
          <Row className="g-3">
            <Col md={6}>
              <Form.Control
                type="text"
                placeholder="Buscar por cliente..."
                value={busquedaCliente}
                onChange={(e) => setBusquedaCliente(e.target.value)}
              />
            </Col>
            <Col md={4}>
              <Form.Control
                type="date"
                value={busquedaFecha}
                onChange={(e) => setBusquedaFecha(e.target.value)}
              />
            </Col>
          </Row>
        </Form>

        {/* 📋 Tabla */}
        <Table striped bordered hover variant="dark" responsive className="text-center">
          <thead>
            <tr>
              <th>#</th>
              <th>Cliente</th>
              <th>Total (COP)</th>
              <th>Fecha</th>
              <th>Estado</th>
              <th>Acciones</th>
            </tr>
          </thead>
          <tbody>
            {cotizacionesFiltradas.length > 0 ? (
              cotizacionesFiltradas.map((c) => (
                <tr key={c.id}>
                  <td>{c.numero}</td>
                  <td>{c.cliente}</td>
                  <td>${c.total.toLocaleString()}</td>
                  <td>{c.fecha}</td>
                  <td>{getBadge(c.estado)}</td>
                  <td>
                    <Button
                      size="sm"
                      variant="outline-light"
                      className="me-2"
                      onClick={() => handleReimprimir(c)}
                    >
                      🖨️ Reimprimir
                    </Button>
                    <Button
                      size="sm"
                      variant="outline-danger"
                      onClick={() => handleDuplicar(c)}
                    >
                      📄 Duplicar
                    </Button>
                  </td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan="6" className="text-center text-muted">
                  No se encontraron cotizaciones.
                </td>
              </tr>
            )}
          </tbody>
        </Table>
      </Card>
    </Container>
  );
}
